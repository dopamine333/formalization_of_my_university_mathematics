...
-- equiv to mathlib compact
theorem compact_via_finite_subcover_iff_IsCompact
  {X : Type} [MetricSpace X] (S : Set X) :
  compact_via_finite_subcover S ↔ IsCompact S := by
...
def compact_exists_isGreatest {K : Set ℝ}
  (hK : compact_via_finite_subcover K) (hK_ne : K.Nonempty) :
  ∃ y_max, IsGreatest K y_max := sorry

def compact_exists_isLeast {K : Set ℝ}
  (hK : compact_via_finite_subcover K) (hK_ne : K.Nonempty) :
  ∃ y_min, IsLeast K y_min := sorry

theorem thm_9_4'
  {X : Type} [MetricSpace X]
  {f : X → ℝ} {K : Set X} (hK : compact_via_finite_subcover K) (hf : Continuous f) (hK_ne : K.Nonempty) :
  ∃ x_min x_max, x_min ∈ K ∧ x_max ∈ K ∧
    (∀ x ∈ K, f x_min ≤ f x) ∧ (∀ x ∈ K, f x ≤ f x_max) := by
  -- f '' K is compact since f is continuous on K
  have h_image_compact : compact_via_finite_subcover (f '' K) := thm_9_3_2 hK hf
  --thm_9_3_2 hK hf
  -- f '' K is nonempty
  have h_fK_nonempty : (f '' K).Nonempty := hK_ne.image f
  -- Compact sets in ℝ have greatest/least elements
  obtain ⟨y_max, hy_max_mem, h_is_max⟩ := compact_exists_isGreatest h_image_compact h_fK_nonempty
  obtain ⟨y_min, hy_min_mem, h_is_min⟩ := compact_exists_isLeast h_image_compact h_fK_nonempty
