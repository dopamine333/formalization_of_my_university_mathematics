...
inductive Proof : List (Formula) → Formula → Type
  ...
  | impI Γ A B : Proof (List.cons A Γ) B → Proof Γ (A.imp B)
...
def weakening {Γ Γ' A} (h : Γ ⊆ Γ'): Proof Γ A → Proof Γ' A
  | axm Γ A h' => by
    ...
    exact this
  | impI Γ A B p => by
    let p' : Proof (List.cons A Γ') B := by
      have h' : List.cons A Γ ⊆ List.cons A Γ' := by
        apply List.cons_subset_cons _ h
      apply weakening h'
      exact p
    apply impI
    exact p'
...
--   | impI Γ A B p => impI _ _ _ (p.weakening (List.cons_subset_cons h))
--   | impE Γ A B p1 p2 => impE _ _ _ (p1.weakening h) (p2.weakening h)
--   | not_not Γ A p => not_not Γ' A (weakening h p)

def weakening1 {Γ A B} : Proof Γ B → Proof (List.cons A Γ) B := by
  intro h
  apply weakening _ h
  apply List.subset_cons_self

def weakening2 {Γ A B C} : Proof (List.cons A Γ) C → Proof (List.cons A (List.cons B Γ)) C := by
  intro h
  apply weakening _ h
  apply List.cons_subset_cons
  apply List.subset_cons_self

def axm1 {Γ A} : Proof (List.cons A Γ) A := by
  apply axm
  apply List.mem_cons_self

def axm2 {Γ A B} : Proof (List.cons A (List.cons B Γ)) B := by
  apply axm
...
def orI1 {Γ A B} : Proof Γ A → Proof Γ (A.or B) := by
  ...
  apply impI
  apply exfalso
  let Γ' := List.cons A.not Γ
  have h : Γ ⊆ Γ' := by apply List.subset_cons_self
  let p1 : Proof Γ' A := by
    apply weakening h p
...
def orE {Γ A B C} : Proof Γ (.or A B) → Proof Γ (A.imp C) → Proof Γ (B.imp C) → Proof Γ C := by
  ...
  apply not_not
  apply impI
  have p1 : Proof (List.cons C.not Γ) C.not := axm _ _ (List.mem_cons_self)
  have p2 : Proof (List.cons C.not Γ) (B.imp C) := weakening1 pB
  have p3 : Proof (List.cons C.not Γ) (A.or B) := weakening1 p
...
def andI {Γ A B} : Proof Γ A → Proof Γ B → Proof Γ (A.and B) := by
  ...
  have p1 : Proof (List.cons (A.imp B.not) Γ) A := pA.weakening1
  have p2 : Proof (List.cons (A.imp B.not) Γ) (A.imp B.not) := axm1
  have p3 : Proof (List.cons (A.imp B.not) Γ) B.not := impE _ _ _ p2 p1
  have p4 : Proof (List.cons (A.imp B.not) Γ) B := pB.weakening1
  have p5 : Proof (List.cons (A.imp B.not) Γ) .falsum := impE _ _ _ p3 p4
  exact p5
...
def andE1 {Γ A B} : Proof Γ (A.and B) → Proof Γ A := by
  ...
  have p1 : Proof (List.cons A (List.cons A.not Γ)) A := axm1
  have p2 : Proof (List.cons A (List.cons A.not Γ)) A.not := axm2
